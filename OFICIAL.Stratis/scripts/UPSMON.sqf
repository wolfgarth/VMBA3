...
// track unit ======================================================================================
	...
	if (_track=="TRACK") then {
		...
		_markerobj setMarkerShape "ICON";
		_markertype = if (isClass(configFile >> "cfgMarkers" >> "WTF_Flag")) then {"WTF_FLAG"} else {"hd_arrow"};
	...
	if (_areatrigger!="NOTRIGGER") then {
		...
		if (isNil ("KRON_TRGFlag")) then {
			...
			if (_areatrigger!="SILENTTRIGGER") then {
				call compile format["%1 setTriggerStatements['count thislist<=%6', 'titletext [''SECTOR <%2> CLEARED'',''PLAIN''];''%2'' setmarkerpos [-%4,-%5];%3=true;', 'titletext [''SECTOR <%2> HAS BEEN REOCCUPIED'',''PLAIN''];''%2'' setmarkerpos [%4,%5];%3=false;']", _trgname,_areamarker,_flgname,_centerX,_centerY,_zoneempty];
			...
			if (_gothit && !_fortify && !_nowp) then { 
				if ((_fightmode == "walk")) then 
				{
					...
					if ((_react >= KRON_UPS_react && _lastreact >=_minreact && count _targets <= 0) || _sowounded) then {										
						...
						if (!_supressed && (random 100 < 60) && !_nosmoke) then {	
							nul= [_npc,_target] spawn MON_throw_grenade;
						};									
						_gothit = false; 
						_makenewtarget = true;		
						...
						if ( count _unitsIn > 0) then {	
							...
							if ( "Air" countType [vehicle (_npc)]>0) then {											
								...
								if (_GetIn_NearestVehicles) then { 
									...
									[vehicle _npc, _TargetPos, _GetOutDist, 90] spawn MON_landHely; // org _flyInHeight shay_gman changed from MON_doParadrop to MON_landHely
									sleep 1;
