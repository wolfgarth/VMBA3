...
BTC_lift_check =
{
	...
	if (_can_lift && ((BTC_cargo isKindOf "Air" && getdammage BTC_cargo != 1) || (format ["%1", _x getVariable "BTC_cannot_lift"] == "1"))) then {_can_lift = false;};
...
[] spawn
{
	...
	if (BTC_def_hud == 1) then {player addAction [("<t color=""#ED2744"">" + ("Hud On\Off") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], 8, true, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
	if (BTC_def_pip == 1) then {player addAction [("<t color=""#ED2744"">" + ("Camera On\Off") + "</t>"),BTC_dir_action, [[],BTC_l_camera], 9, true, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip"];};
